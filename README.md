...
- **证件照工作流**：Fpter 滤镜模块提供灰度 / 动漫模式滤镜 + 滤镜窗口；Fptm 工作流模块提供换底色 / 智能裁切 / 网格排版
...
plugins/
├── Fpter/                 内置滤镜模块（灰度/动漫滤镜 + 滤镜窗口；module.json + ALC 加载）
├── Fptm/                  传统面板 + 证件照工作流（换底色/智能裁切/排版）
├── FpEdit/                编辑模块（复制/粘贴/撤销/重做 + 像素剪贴板）
└── Itool/                 画笔工具模块（9 个编辑工具 + 画笔窗口）
