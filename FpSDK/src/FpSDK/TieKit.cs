...
{
    /// <summary>绑定的 tie 发行版：Harbor 2026.1 预发布5（tink 帧协议/std/zd 进入标准库）。</summary>
    public const string Harbor = "Harbor-2026.1-preview.5";

    /// <summary>tie 插件运行桥协议版本（宿主 ↔ tie 脚本）。v2 = tink 帧桥（stdin/stdout 行帧流）。</summary>
    public const string BridgeProtocol = "fptp.tie-bridge.v2";
}
...
/// <para>协议 v2（tink 帧桥，templates/tie-module 同款）：</para>
/// <list type="bullet">
...
/// </list>
/// 仅使用 tie 内联底座（get_env/println/str_len/str_char/算术）+ std/tink.tie 帧层，零 tie-interp 依赖。
/// tiec.exe 定位：<c>FPTP_TIE_HOME</c> 环境变量，或 <c>&lt;运行目录&gt;/tools/tie/tiec.exe</c>（随 NuGet 分发）；
/// 编译所需 LLVM 按 tiec 标准回退链发现（同级 llvm / PATH / TIE_LLVM_HOME）。
/// </summary>
public static class TieRunner
{
    ...
    public static TieResult Run(string scriptPath, string input, int timeoutMs = 60_000)
    {
        ...
        try
        {
            ...
            //    tiec 依赖同版本 LLVM：优先 tiec 同级 llvm/ 或 FPTP_TIE_HOME 发行根 bin/llvm，
            //    显式设置 TIE_LLVM_HOME，避免静默回退到 PATH/D:\LLVM 旧版 LLVM
            //    （旧版 LLVM 链接出的 exe（read_line → tie-interp）会静默无输出）。
