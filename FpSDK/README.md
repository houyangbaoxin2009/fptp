...
# 生成：main.tie + fptp_sdk.tie + std/tink.tie + rdu/crc.tie + module.json（type=script）+ langs/
```

写插件只需实现 `process`（v2 帧桥协议已抽到 `fptp_sdk.tie` 库）：
...
func main() {
    fptp.bridge(process)   // 读 stdin 帧 → process → OK 应答帧
}
```
...
```csharp
// C# 侧：编译并运行 main.tie，以文本进出（自动 tink 帧编解码）
...
│   └── TieKit.cs                # tie 集成：TieVersion + TieRunner（编译运行桥，v2 tink 帧桥）
├── cli/FpSdkCli/                # fpsdk 脚手架（dotnet tool，命令 fpsdk）
├── tools/tie/                   # tie 运行时产物（tiec.exe，Harbor-2026.1-preview.5，随包分发）
├── templates/dotnet-module/     # .NET 插件项目模板（token 化）
├── templates/tie-module/        # tie 脚本插件模板（main.tie + fptp_sdk.tie 运行桥库 + std/tink.tie 帧层 + module.json type=script）
