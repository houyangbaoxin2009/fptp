...
- **fptm 一键证件照向导**：传统面板新增「一键证件照」区——选尺寸预设 + 目标底色 + 容差/羽化 + 相纸/拼版，
  单步串联智能裁切 → 换底色 → 可选排版（不排版直接生成证件照，排版生成相纸新文档），参数即时写回设置
- **fptm 红眼去除**：检测高红瞳孔（红色显著高于绿蓝）去红压暗，阈值/强度可调（设置组 + 图像菜单命令）
- **fptm 拼版模板**：新增 6寸·1寸×8（4×2）/ 6寸·2寸×4（2×2）固定网格排版预设，
  照片自动缩放到标准证件照尺寸后排布；排版设置下拉同步可选
...
### 新增
- **四边停靠面板（DockHost）**：模组面板可停靠到窗口上/左/下/右四边，`DockManager` 管理停靠区与标签页，`AddPanelInternal` 按 `PanelSide` 自动归类（替代固定左右两栏布局）
- **动漫模式滤镜**：`fptp.builtin.anime`，照片转动漫（色彩量化 + 边缘提取），参数 Levels（量化级数）/ Outline（边缘强度）/ Smooth（平滑半径），经"图像/动漫模式"菜单或批量处理调用
- **换底增强：任意颜色 + 背景图片**：新背景可选任意颜色（ColorPicker 弹系统颜色对话框）或图片（BackgroundImage 路径，经 CodecRegistry 解码铺满，优先于纯色）
- **换底动漫角色模式**：Mode 参数新增动漫模式——边缘感知 flood fill 抠图（背景色种子向内部蔓延，Sobel 描边阈值 EdgeThreshold 阻断强边缘），适配硬边色块与描边线，复杂背景也能整体抠出，角色主体不被误换
- **参数控件扩展**：`FilterParameterKind` 新增 `ColorPicker`（颜色对话框 + 色块预览）与 `Image`（路径文本框 + 浏览按钮）两类参数，壳按类型自动渲染控件

### 修复
- **画布缩放中间帧抖动（鬼畜）**：缩放时 AutoScrollMinSize 变化触发滚动条重算导致中间帧跳动。缩放全程 `WM_SETREDRAW` 锁绘（`SetRedraw`），完成一次性刷新；同时修正缩放偏移计算（锚点像素随视口正确跟随）
- **排版大图崩溃**：照片大于相纸时等比缩小后再排版，居中起点非负不再越界
- **套索越界写蒙版**：鼠标捕获后拖出画布，套索坐标钳制到文档范围（`ClampToDoc`），防越界写蒙版；保留闭合多边形供 `DrawOverlay` 持续绘制选区蚂蚁线
- **重做顺序不对称**：重做与撤销严格反序——先前进到下一个文档（排版/裁切撤销后回原图的情形），无文档级可重做时再重放当前文档内历史，修复 Ctrl+Y 无反应

### 变更
- 版本号提升至 2.0.10.0（全部项目与内置模组同步）
