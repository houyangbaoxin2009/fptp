...
/// <summary>
/// 工具宿主服务（壳实现，注册进 Services）：聚合各模块（IToolPlugin）的交互工具，
/// ActivateTool 把指定工具设为画布当前激活工具（经画布 VM）。
...
internal sealed class ToolHostService : IToolHostService
{
    ...
    /// <summary>登记某模块（IToolPlugin）贡献的全部工具（重复 Id 覆盖）。</summary>
    public void RegisterModule(IToolPlugin module)
    {
