...
/// modules.json / settings.json / secure.json 与扩展模块安装目录全部与 GUI 相同——
...
internal static class CliEnvironment
{
    ...
    /// <summary>模块状态注册表：{模块Id: "enabled"|"disabled"|"removed"}，与 GUI 同文件。</summary>
    public static string ModulesPath => Path.Combine(AppDataDir, "modules.json");

    /// <summary>模块配置（User/Core 段）：{模块Id.键: 值}，与 GUI 同文件。</summary>
    public static string SettingsPath => Path.Combine(AppDataDir, "settings.json");

    /// <summary>安全配置（Security 段，仅更新模块可写、独立文件隔离），与 GUI 同文件。</summary>
