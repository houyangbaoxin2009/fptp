...
/// <summary>tie 脚本执行结果（协议解析结果，FpSDK.TieRunner 同构）。</summary>
...
/// （同目录 fptp_sdk.tie 等依赖随拷）并执行，以协议文本进出（自动 base64 编解码）。
/// <para>协议（与 FpSDK.TieRunner / templates/tie-module 同款）：</para>
/// <list type="bullet">
///   <item>输入：环境变量 <c>FPTP_TIE_INPUT</c> = base64(宿主协议文本)；</item>
...
/// 脚本仅用 tie 内联底座（get_env/println/字符串/算术/table 原语），零 tie-interp 依赖（file 桥需 tie_interp.lib，未随包）。
/// </summary>
public static class TieRunner
{
    ...
    /// 脚本约定顶层 <c>func main()</c> 调 fptp.input()/fptp.reply_ok(...)（见 fptp_sdk.tie 模板）。
    /// </summary>
