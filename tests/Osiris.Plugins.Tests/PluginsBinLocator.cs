...
/// （宿主运行时扫描目录，含 Fpter 子目录的 Fpter.dll）。
/// </summary>
    ...
    public static string Path
    {
        get
        {
            ...
            while (dir is not null)
            {
                ...
                if (File.Exists(System.IO.Path.Combine(candidate, "Fpter", "Fpter.dll")))
                    return candidate;
                dir = dir.Parent;
            }
            throw new InvalidOperationException("未找到仓库 plugins/bin 目录（请先构建 Fpter 项目）。");
        }
    }
}
